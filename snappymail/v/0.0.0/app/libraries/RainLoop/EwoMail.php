...
class EwoMail
{
     ...
     * 修改密码
     */
    public function updatePassword(string $email, string $password, string $new_password): mixed
    {
        $newData = [
            'email' => $email,
            'password' => $password,
    ...
    public function getDomain(string $domain): ?array
    {
        $data = null;
        if ($domain == '') {
           $domain = \RainLoop\Api::Config()->Get('login', 'default_domain', '');
        }
     ...
     * 检查是否有控制面板登陆权限
     */
    public function is_login_panel(string $username, string $password): int
    {
        $newData = [
            'username' => $username,
            'password' => $password,
        ];
        $r = $this->send('Admin/is_login_panel', $newData);
        return $r['status'];
    }
